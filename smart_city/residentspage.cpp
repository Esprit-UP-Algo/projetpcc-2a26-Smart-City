    ...
    , ui(new Ui::ResidentsPage)
{
    ...
    if (ui->sexeCombo) ui->sexeCombo->setVisible(false);
    if (ui->sexeLabel) ui->sexeLabel->setVisible(false);
...
void ResidentsPage::setupTableHeaders()
{
    ...
    t->setColumnCount(9);
    QStringList headers{
        tr("CIN"), tr("Nom"), tr("Prénom"), tr("Date de naissance"),
        tr("Adresse"), tr("Téléphone"), tr("Email"), tr("Statut"),
        tr("Satisfaction")
    };
...
void ResidentsPage::onSaveForm()
{
    ...
    data["date_naissance"] = ui->dateEdit->date().toString("yyyy-MM-dd");
    data["adresse"]     = ui->appartementEdit->text().trimmed();
    ...
    data["satisfaction"] = ui->etageLineEdit->text().trimmed().toInt();
...
int ResidentsPage::sortColumnForIndex(int index) const
{
    ...
    case 0: return 0; // CIN
    case 1: return 1; // Nom
    case 2: return 4; // Adresse
    case 3: return 3; // Date de naissance
    default: return 0;
    }
}
...
void ResidentsPage::loadResident(const QVariantMap &r)
{
    ...
    ui->dateEdit->setDate(QDate::fromString(r.value("date_naissance").toString(), "yyyy-MM-dd"));
    ui->appartementEdit->setText(r.value("adresse").toString());
    ui->etageLineEdit->setText(r.value("satisfaction").toString());
...
bool ResidentsPage::validateForm()
{
    ...
    if (!ui->dateEdit->date().isValid() || ui->dateEdit->date() > QDate::currentDate()) {
        showError("Erreur : Date de naissance invalide");
        return false;
    }
    
    // Check Adresse
    if (ui->appartementEdit->text().trimmed().isEmpty()) {
        showError("Erreur : Le champ Adresse est vide");
        return false;
    }
    
    // Check Satisfaction
    bool ok = false;
    int satisfaction = ui->etageLineEdit->text().trimmed().toInt(&ok);
    if (!ok || satisfaction < 0 || satisfaction > 100) {
        showError("Erreur : Satisfaction doit être un nombre entre 0 et 100");
        return false;
    }
