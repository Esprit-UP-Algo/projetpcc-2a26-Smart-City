...
QList<QVariantMap> DatabaseManager::getAllResidents()
{
    ...
    if (!q.exec(sql)) {
        qWarning() << "[Oracle] getAllResidents ERROR:" << q.lastError().text();
        return list;
    }
...
QVariantMap DatabaseManager::getResident(const QString &cin)
{
    ...
    if (q.exec() && q.next()) {
        r["cin"]         = q.value("CIN");
        r["nom"]         = q.value("NOM");
        r["prenom"]      = q.value("PRENOM");
        r["sexe"]        = q.value("SEXE");
        r["telephone"]   = q.value("TELEPHONE");
        r["email"]       = q.value("EMAIL");
        r["date_entree"] = q.value("DATE_ENTREE");
        r["appartement"] = q.value("APPARTEMENT");
        r["etage"]       = q.value("ETAGE");
        r["statut"]      = q.value("STATUT");
    }
...
bool DatabaseManager::addResident(const QVariantMap &data)
{
    ...
    q.prepare(R"(
        ...
        VALUES (:cin, :nom, :prenom, :sexe, :tel, :email,
                TO_DATE(:date_entree,'YYYY-MM-DD'),
                :app, :etage, :statut)
    )");
...
bool DatabaseManager::updateResident(const QString &cin, const QVariantMap &data)
{
    ...
    if (!q.exec()) {
        qWarning() << "[Oracle] updateResident ERROR:" << q.lastError().text();
        return false;
    }
...
bool DatabaseManager::deleteResident(const QString &cin)
{
    ...
    if (!q.exec()) {
        qWarning() << "[Oracle] deleteResident ERROR:" << q.lastError().text();
        return false;
    }
...
QList<QVariantMap> DatabaseManager::searchResidents(const QString &term)
{
    ...
    q.bindValue(":t", pattern);

    if (!q.exec()) {
        qWarning() << "[Oracle] searchResidents ERROR:" << q.lastError().text();
        return list;
    }
...
QVariantMap DatabaseManager::getStatistics()
{
    ...
    if (q.exec("SELECT COUNT(*) FROM RESIDENTS") && q.next())
        stats["total"] = q.value(0).toInt();

    if (q.exec("SELECT COUNT(*) FROM RESIDENTS WHERE UPPER(STATUT)='LOCATAIRE'") && q.next())
        stats["locataires"] = q.value(0).toInt();

    if (q.exec("SELECT COUNT(*) FROM RESIDENTS WHERE UPPER(STATUT)='PROPRIETAIRE' OR UPPER(STATUT)='PROPRIÉTAIRE'") && q.next())
        stats["proprietaires"] = q.value(0).toInt();
...
bool DatabaseManager::createHistoriqueTable()
{
    if (!db.isOpen()) {
        return false;
    }

    // Essayer de créer la table sans vérification préalable
    QSqlQuery query(db);
    QString createTable = R"(
        CREATE TABLE historique_vehicules (
            ...
            id_vehicule VARCHAR2(10) NOT NULL,
            type VARCHAR2(50),
            capacite INTEGER,
            zone VARCHAR2(100),
            horaire VARCHAR2(20),
            statut VARCHAR2(20),
            date_ajout DATE,
            temps_utilise NUMBER(10,2),
            code_unique VARCHAR2(50),
            notes VARCHAR2(500)
        )
    )";

    if (query.exec(createTable)) {
        qDebug() << "✅ Table historique_vehicules créée avec succès";
        return true;
    } else {
        // En cas d'erreur, on suppose que la table existe déjà (le cas le plus fréquent)
        qDebug() << "ℹ️ Table historique_vehicules existe probablement déjà";
        return true; // On retourne true car la table est utilisable
    }
}

bool DatabaseManager::addHistoriqueVehicule(const QVariantMap &historique)
{
    if (!db.isOpen()) {
        qDebug() << "❌ Base non connectée pour ajouter historique";
        return false;
    }
    ...
    if (maxQuery.exec("SELECT NVL(MAX(id), 0) + 1 FROM historique_vehicules") && maxQuery.next()) {
        id = maxQuery.value(0).toInt();
    }

    QSqlQuery query(db);
    query.prepare(R"(
        INSERT INTO historique_vehicules
        (id, action, id_vehicule, type, capacite, zone, horaire, statut, date_ajout, temps_utilise, code_unique, notes)
        VALUES (:id, :action, :id_vehicule, :type, :capacite, :zone, :horaire, :statut,
                TO_DATE(:date_ajout,'YYYY-MM-DD'), :temps_utilise, :code_unique, :notes)
    )");

    query.bindValue(":id", id);
    query.bindValue(":action", historique.value("action").toString());
    query.bindValue(":id_vehicule", historique.value("id_vehicule").toString());
    query.bindValue(":type", historique.value("type"));
    query.bindValue(":capacite", historique.value("capacite"));
    query.bindValue(":zone", historique.value("zone"));
    query.bindValue(":horaire", historique.value("horaire"));
    query.bindValue(":statut", historique.value("statut"));
    query.bindValue(":date_ajout", historique.value("date_ajout"));
    query.bindValue(":temps_utilise", historique.value("temps_utilise"));

    QString code_unique = historique.value("code_unique").toString();
    if (code_unique.isEmpty()) {
        query.bindValue(":code_unique", QVariant());
    } else {
        query.bindValue(":code_unique", code_unique);
    ...
    query.bindValue(":notes", historique.value("notes", ""));

    if (!query.exec()) {
        qDebug() << "❌ Erreur ajout historique:" << query.lastError().text();
        qDebug() << "❌ Détails:" << query.lastQuery();
        return false;
    }

    db.commit();
    qDebug() << "✅ Historique ajouté - ID:" << id << "Véhicule:" << historique.value("id_vehicule").toString();
    return true;
}

QList<QVariantMap> DatabaseManager::getAllHistoriqueVehicules()
{
    QList<QVariantMap> historique;
    if (!db.isOpen()) {
        qDebug() << "❌ Base non connectée pour lire historique";
        return historique;
    }
    ...
    QString sql = R"(
        SELECT id,
               ...
               action, id_vehicule, type, capacite, zone, horaire, statut,
               TO_CHAR(date_ajout, 'YYYY-MM-DD') as date_ajout,
               temps_utilise, code_unique, notes
        FROM historique_vehicules
        ORDER BY timestamp DESC
    )";

    if (!query.exec(sql)) {
        qDebug() << "❌ Erreur lecture historique:" << query.lastError().text();
        return historique;
    }
    ...
    while (query.next()) {
        ...
        entry["timestamp"] = query.value("timestamp");
        entry["action"] = query.value("action");
        entry["id_vehicule"] = query.value("id_vehicule");
        entry["type"] = query.value("type");
        entry["capacite"] = query.value("capacite");
        entry["zone"] = query.value("zone");
        entry["horaire"] = query.value("horaire");
        entry["statut"] = query.value("statut");
        entry["date_ajout"] = query.value("date_ajout");
        entry["temps_utilise"] = query.value("temps_utilise");
        entry["code_unique"] = query.value("code_unique");
        entry["notes"] = query.value("notes");

        historique.append(entry);
    }

    qDebug() << "✅ Historique chargé:" << historique.size() << "entrées";
    return historique;
}

QList<QVariantMap> DatabaseManager::getHistoriqueByVehiculeId(const QString &idVehicule)
{
    QList<QVariantMap> historique;
    if (!db.isOpen()) {
        return historique;
    ...
    query.bindValue(":id_vehicule", idVehicule);
...
QVariantMap DatabaseManager::getResidentByCIN(const QString &cin)
{
    QVariantMap m;
    if (!db.isOpen())
        return m;
