...
bool DatabaseManager::addResident(const QVariantMap &data)
{
    ...
    // --- Correction pour CK_RESIDENTS_SEXE ---
    ...
    if (sexe == "homme")
        ...
        sexe = "Homme"; // valeur par défaut valide
...
bool DatabaseManager::updateResident(const QString &cin, const QVariantMap &data)
{
    ...
    // --- 🔧 Correction pour la contrainte CK_RESIDENTS_SEXE ---
    QString sexe = data.value("sexe").toString().trimmed().toLower();
    if (sexe == "homme")
        sexe = "Homme";
    else if (sexe == "femme")
...
QList<QVariantMap> DatabaseManager::getAllResidents()
{
    ...
    q.exec(R"(
        SELECT CIN, NOM, PRENOM, SEXE, TELEPHONE, EMAIL,
               ...
               APPARTEMENT, ETAGE, STATUT
        FROM RESIDENTS ORDER BY DATE_ENTREE DESC
    )");
    while (q.next()) {
        QVariantMap m;
        m["cin"] = q.value("CIN");
...
QString DatabaseManager::getResidentIdFromCin(const QString &cin)
{
    ...
    return QString(); // vide = pas trouvé
}
...
bool DatabaseManager::addTransaction(const QVariantMap &data)
{
    ...
    QString date        = normalizeDate(data.value("date_transaction"));
    ...
    // --------------------------------------------------------
    // 🔍 Convertir le CIN → ID_RESIDENT
    // --------------------------------------------------------
    QVariant idResident = QVariant();  // NULL si vide
...
QList<QVariantMap> DatabaseManager::getAllTransactions()
{
    QList<QVariantMap> list;
    if (!db.isOpen()) {
        qWarning() << "❌ Database not open for getAllTransactions";
        return list;
    }

    // First, let's check what columns exist in the table
    QSqlQuery checkTable(db);
    if (checkTable.exec("SELECT * FROM TRANSACTIONS WHERE ROWNUM <= 1")) {
        QSqlRecord record = checkTable.record();
        qInfo() << "📊 TRANSACTIONS table has" << record.count() << "columns:";
        for (int i = 0; i < record.count(); ++i) {
            qInfo() << "  Column" << i << ":" << record.fieldName(i);
        }
    }

    QSqlQuery q(db);
    // Try a simpler query first
    if (!q.exec("SELECT COUNT(*) FROM TRANSACTIONS")) {
        qWarning() << "❌ Cannot count TRANSACTIONS:" << q.lastError().text();
        return list;
    }
    
    if (q.next()) {
        int count = q.value(0).toInt();
        qInfo() << "📊 Found" << count << "transactions in database";
        if (count == 0) {
            qInfo() << "ℹ️ No transactions found in TRANSACTIONS table";
            return list;
        }
    }
    ...
    if (!q.exec(R"(
        ...
        qWarning() << "❌ getAllTransactions full query failed:" << q.lastError().text();
        
        // Try a simpler query without date formatting
        if (!q.exec("SELECT * FROM TRANSACTIONS ORDER BY DATE_TRANSACTION DESC")) {
            qWarning() << "❌ getAllTransactions simple query failed:" << q.lastError().text();
            return list;
        }
    }
    ...
    while (q.next()) {
        ...
        // Get data using column indices to avoid case sensitivity issues
        QSqlRecord record = q.record();
        for (int i = 0; i < record.count(); ++i) {
            QString fieldName = record.fieldName(i).toLower();
            m[fieldName] = q.value(i);
        }
        
        // Also try the original field names
    ...
    qInfo() << "✅ Successfully loaded" << rowCount << "transactions";
    return list;
}
...
bool DatabaseManager::clearAllResidents()
{
    ...
    db.commit();
    qInfo() << "✅ All residents cleared successfully";
    emit dataChanged();
    return true;
}
...
bool DatabaseManager::clearAllTransactions()
{
    ...
    db.commit();
    qInfo() << "✅ All transactions cleared successfully";
    emit dataChanged();
    return true;
}
...
bool DatabaseManager::clearAllIncidents()
{
    ...
    db.commit();
    qInfo() << "✅ All incidents cleared successfully";
    emit dataChanged();
    return true;
}
...
bool DatabaseManager::clearAllLocaux()
{
    ...
    db.commit();
    qInfo() << "✅ All locaux cleared successfully";
    emit dataChanged();
    return true;
}
...
bool DatabaseManager::clearAllTransportVehicules()
{
    ...
    db.commit();
    qInfo() << "✅ All transport vehicles cleared successfully";
    emit dataChanged();
    return true;
}
...
bool DatabaseManager::clearAllData()
{
    ...
    // Clear all tables in the correct order (considering foreign keys)
    QStringList queries = {
    ...
    for (const QString& query : queries) {
        if (!q.exec(query)) {
            qWarning() << "❌ clearAllData:" << q.lastError().text() << "for query:" << query;
            db.rollback();
            return false;
        }
    }
    ...
    db.commit();
    qInfo() << "✅ All data cleared successfully";
    emit dataChanged();
    return true;
}
