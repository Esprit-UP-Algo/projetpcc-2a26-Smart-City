...
bool DatabaseManager::addLocal(const QVariantMap &data)
{
    ...
    QSqlQuery q(db);
    q.prepare(R"(
        INSERT INTO LOCAUX (CODE_UNIQUE, NOM, ADRESSE, NUM_LOCAUX, SUPERFICIE, "TYPE", STATUT)
        VALUES (:code, :nom, :adresse, :numero, :superficie, :type, :statut)
    )");
    q.bindValue(":code", data.value("code_unique").toString().trimmed());
...
bool DatabaseManager::deleteLocal(const QString &id)
{
    ...
    QSqlQuery q(db);
    q.prepare("DELETE FROM LOCAUX WHERE CODE_UNIQUE=:code");
    q.bindValue(":code", id);
    if (!q.exec()) {
...
QList<QVariantMap> DatabaseManager::getAllLocaux()
{
    ...
    QSqlQuery q(db);
    if (!q.exec(R"(
            SELECT CODE_UNIQUE, NOM, ADRESSE, NUM_LOCAUX, SUPERFICIE, "TYPE", STATUT
            FROM LOCAUX
            ORDER BY NOM
        )")) {
...
bool DatabaseManager::addTransportVehicule(const QVariantMap &data)
{
    ...
    // Gérer code_unique : NULL si vide, sinon vérifier qu'il existe dans LOCAUX
    if (code_unique.isEmpty()) {
        query.bindValue(":code_unique", QVariant());
    } else {
        // Vérifier que le local existe
        QSqlQuery checkQuery(db);
        checkQuery.prepare("SELECT COUNT(*) FROM LOCAUX WHERE CODE_UNIQUE = :code");
        checkQuery.bindValue(":code", code_unique);
        if (checkQuery.exec() && checkQuery.next() && checkQuery.value(0).toInt() == 0) {
            QMessageBox::warning(nullptr, "Erreur de validation",
                                QString("Le code local '%1' n'existe pas dans la base de données.\n"
                                        "Veuillez d'abord créer le local ou laisser ce champ vide.").arg(code_unique));
            return false;
        }
