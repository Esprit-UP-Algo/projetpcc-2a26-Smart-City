...
void TransactionDialog::setupUi()
{
    ...
    connect(saveButton, &QPushButton::clicked, this, &TransactionDialog::onSaveClicked);
...
void TransactionDialog::onSaveClicked()
{
    ...
    QString categorie = categorieCombo->currentText().trimmed();
    ...
    if (code.isEmpty()) {
        QMessageBox::warning(this, "Erreur de saisie", "⚠️ Le champ 'Code' est obligatoire !");
        return;
    }

    if (montant <= 0) {
        QMessageBox::warning(this, "Erreur de saisie", "⚠️ Le montant doit être supérieur à 0 !");
        return;
    }

    if (categorie.isEmpty()) {
        QMessageBox::warning(this, "Erreur de saisie", "⚠️ La catégorie ne peut pas être vide !");
        return;
    }

    if (!date.isValid()) {
        QMessageBox::warning(this, "Erreur de saisie", "⚠️ La date saisie est invalide !");
        return;
    }

    if (type.isEmpty()) {
        QMessageBox::warning(this, "Erreur de saisie", "⚠️ Vous devez choisir un type de transaction !");
        return;
    }

    // ✅ Tout est bon → accepter la boîte de dialogue
    accept();
}
