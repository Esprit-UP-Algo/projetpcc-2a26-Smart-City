...
void TransportPage::genererHistoriqueHTML(const QList<QVariantMap> &historique)
{
    ...
    out << "        }\n";
    out << "        .stat-card.ajout { border-left-color: #A0AEC0; }\n";
    ...
    out << "        .ignored-note {\n";
    out << "            background: #F7FAFC;\n";
    out << "            border-left: 4px solid #A0AEC0;\n";
    out << "            padding: 15px;\n";
    out << "            margin: 20px 0;\n";
    ...
    out << "            <p>Historique des Modifications et Suppressions de Véhicules</p>\n";
    out << "            <div class='timestamp'>\n";
    out << "                Généré le " << QDateTime::currentDateTime().toString("dd/MM/yyyy à HH:mm:ss") << "\n";
    out << "            </div>\n";
    out << "        </div>\n";

    // Calcul des statistiques - MAINTENANT SEULEMENT MODIFICATIONS ET SUPPRESSIONS
    int modifications = 0, suppressions = 0;
    for (const auto &v : historique) {
        QString action = v.value("historique_action").toString();
        if (action == "MODIFICATION") modifications++;
    ...
    out << "                    <div class='stat-number'>-</div>\n";
    out << "                    <div class='stat-label'>Ajouts (ignorés)</div>\n";
    out << "                </div>\n";
    ...
    // Note d'information sur les ajouts ignorés
    out << "        <div class='ignored-note'>\n";
    out << "            <strong>📝 Note :</strong> Les ajouts de nouveaux véhicules ne sont pas enregistrés dans l'historique.\n";
    out << "            Seules les modifications et suppressions sont tracées.\n";
    out << "        </div>\n";
    ...
    out << "            <p>Système de gestion des véhicules de transport - Historique des modifications et suppressions uniquement</p>\n";
    out << "        </div>\n";
    out << "    </div>\n";
    out << "</body>\n";
    out << "</html>\n";
...
{
    // 🔴 NE sauvegarder QUE les modifications et suppressions - IGNORER les ajouts
    if (action == "AJOUT") {
        qDebug() << "📝 Historique: Ignorer l'ajout -" << vehicule["id_vehicule"].toString();
        return; // Ne pas sauvegarder les ajouts
    }

    // Vérifier que c'est bien une modification ou suppression
    if (action != "MODIFICATION" && action != "SUPPRESSION") {
        qDebug() << "⚠️  Action non prise en compte pour l'historique:" << action;
        return;
    }
    ...
    qDebug() << "💾 Sauvegarde historique - Action:" << action << "- Véhicule:" << vehicule["id_vehicule"].toString();
    ...
    // Limiter la taille de l'historique (1000 entrées max)
    ...
    if (fichier.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        ...
        if (bytesEcris > 0) {
            ...
            // Mettre à jour le fichier HTML existant sur le Bureau
...
void TransportPage::onSaveForm()
{
    ...
    if (success) {
        ...
        // 🔴 SAUVEGARDER DANS L'HISTORIQUE UNIQUEMENT LES MODIFICATIONS
        if (isUpdate) {
            sauvegarderHistorique(data, action);
            qDebug() << "📝 Historique: Modification enregistrée pour" << id;
        } else {
            qDebug() << "📝 Historique: Ignorer l'ajout du véhicule" << id;
        }
...
void TransportPage::loadVehicule(const QVariantMap &v)
{
    ...
    // CORRIGÉ : Pour QLineEdit, utilisez setText()
